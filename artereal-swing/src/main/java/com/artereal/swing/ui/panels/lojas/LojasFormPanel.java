...
public class LojasFormPanel extends JPanel {
    ...
    /**
     * Cria campos de identificação otimizados
     */
    private JPanel createIdentificationFields() {
        JPanel content = new JPanel();
        content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS));
        content.setBackground(Color.WHITE);
        
        // Primeira linha: Código e Nome (campos principais)
        JPanel row1 = createEnhancedFieldRow(
            createFieldPanel("🔢 Código:", codigoField),
            createFieldPanel("📛 Nome:", nomeField)
        );
        
        // Segunda linha: Número e Data Fundação
        JPanel row2 = createEnhancedFieldRow(
            createFieldPanel("🏢 Número:", numeroField),
            createFieldPanel("📅 Data Fundação:", dataFundacaoField)
        );
    ...
    /**
     * Cria campos de informações maçônicas otimizados
     */
    private JPanel createMasonicFields() {
        JPanel content = new JPanel();
        content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS));
        content.setBackground(Color.WHITE);
        
        JPanel row = createEnhancedFieldRow(
            createFieldPanel("🔷 Ritual:", ritualField),
            createFieldPanel("⭐ Potência:", poderField)
        );
    ...
    /**
     * Cria campos de endereço otimizados
     */
    private JPanel createAddressFields() {
        JPanel content = new JPanel();
        content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS));
        content.setBackground(Color.WHITE);
        
        // Linha 1: Endereço (completo)
        JPanel row1 = createFullWidthFieldPanel("🏠 Endereço:", enderecoField);
        
        // Linha 2: Bairro e Cidade
        JPanel row2 = createEnhancedFieldRow(
            createFieldPanel("🏘️ Bairro:", bairroField),
            createFieldPanel("🏙️ Cidade:", cidadeField)
        );
        
        // Linha 3: Estado e CEP
        JPanel row3 = createEnhancedFieldRow(
            createFieldPanel("🗺️ Estado:", estadoField),
            createFieldPanel("📮 CEP:", cepField)
        );
    ...
    /**
     * Cria campos de contato otimizados
     */
    private JPanel createContactFields() {
        JPanel content = new JPanel();
        content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS));
        content.setBackground(Color.WHITE);
        
        JPanel row = createEnhancedFieldRow(
            createFieldPanel("📞 Telefone:", telefoneField),
            createFieldPanel("📧 E-mail:", emailField)
        );
    ...
    /**
     * Cria campos de diretoria otimizados
     */
    private JPanel createDirectorshipFields() {
        JPanel content = new JPanel();
        content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS));
        content.setBackground(Color.WHITE);
        
        // Linha 1: Presidente (cargo principal)
        JPanel row1 = createFullWidthFieldPanel("👑 Presidente:", presidenteField);
        
        // Linha 2: Secretário e Tesoureiro
        JPanel row2 = createEnhancedFieldRow(
            createFieldPanel("📝 Secretário:", secretarioField),
            createFieldPanel("💰 Tesoureiro:", tesoureiroField)
        );
    ...
    /**
     * Cria campos de observações otimizados
     */
    private JPanel createObservationsFields() {
        JPanel container = new JPanel(new BorderLayout());
        container.setBackground(Color.WHITE);
    ...
    /**
     * Cria linha de campos com estilo aprimorado
     */
    private JPanel createEnhancedFieldRow(JPanel... fieldPanels) {
        JPanel row = new JPanel(new FlowLayout(FlowLayout.LEFT, 25, 5));
        row.setBackground(Color.WHITE);
        
        for (JPanel fieldPanel : fieldPanels) {
            // Adicionar espaçamento visual entre os campos
            fieldPanel.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 10));
            row.add(fieldPanel);
        }
    ...
    /**
     * Cria linha de campos (mantido para compatibilidade)
     */
    private JPanel createFieldRow(JPanel... fieldPanels) {
        return createEnhancedFieldRow(fieldPanels);
    }
    
    /**
     * Cria painel de campo individual
     */
